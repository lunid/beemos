                                ...
                                if(!$ver_filtro){
                                    ...
                                    if(mysql_num_rows($rs) <= 0){
                                        ...
                                        while($row = mysql_fetch_object($rs)){
                                            $rs_saldo = calculaSaldo($row->ID_CLIENTE, $ret->ID_CLIENTE);

                                            $dados .= "<usuario>";
                                                $dados .= "<id_usuario>";
                                                    $dados .= $row->ID_CLIENTE;
                                                $dados .= "</id_usuario>";
                                                $dados .= "<nome>";
                                                    $dados .= utf8_encode($row->NOME_PRINCIPAL);
                                                $dados .= "</nome>";
                                                $dados .= "<email>";
                                                    $dados .= utf8_encode($row->EMAIL);
                                                $dados .= "</email>";
                                                $dados .= "<login>";
                                                    $dados .= utf8_encode($row->LOGIN);
                                                $dados .= "</login>";
                                                $dados .= "<status>";
                                                    $dados .= utf8_encode($row->DEL == 1 ? 'Excluído' : $row->BLOQ == 1 ? 'Bloqueado' : 'Ativo');
                                                $dados .= "</status>";
                                                $dados .= "<dataRegistro>";
